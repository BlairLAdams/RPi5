...
echo "──────────────────────────────────────────────"
cat ~/scr/06_dbt/dbt_project.yml || echo "❌ Missing: dbt_project.yml"

echo -e "\n📁 Inspecting DBT Profile Config:"
echo "──────────────────────────────────────────────"
cat ~/.dbt/profiles.yml || echo "❌ Missing: profiles.yml"

echo -e "\n📁 Listing models in ~/scr/06_dbt/models/:"
echo "──────────────────────────────────────────────"
find ~/scr/06_dbt/models/ -type f -name '*.sql'

echo -e "\n📁 Listing source file (if present):"
echo "──────────────────────────────────────────────"
cat ~/scr/06_dbt/models/_sources.yml 2>/dev/null || echo "⚠️ No _sources.yml found"
